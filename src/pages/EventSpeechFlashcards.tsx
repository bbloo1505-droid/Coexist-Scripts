...
function usePresentationViewportLock(enabled: boolean) {
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      else if (theme) theme.setAttribute("content", "#0c0f0d");
    };
  }, [enabled]);
}
    ...
    return (
      ...
      >
        <div className="mx-auto flex w-full max-w-lg flex-1 flex-col justify-center px-6 py-10">
          ...
          <p className="mt-8 text-center font-sans text-xs font-medium uppercase tracking-[0.16em] text-[#5c6659]">
            Co-Exist · presenter only
          </p>
        </div>
      </div>
    );
